...
import type { GenderBreakdown } from "./types";

type PatientsDemographicsChartProps = {
...
export function PatientsDemographicsChart({ data }: PatientsDemographicsChartProps) {
  const filtered = data
    .filter((d) => d.count > 0)
    .map((d) => ({
          ...
          <PieChart>
            ...
            >
              {filtered.map((entry, index) => (
                <Cell
                  key={`cell-${index}`}
                  fill={genderColors[entry.gender] ?? "var(--chart-2)"}
                />
              ))}
            </Pie>
            <Tooltip
