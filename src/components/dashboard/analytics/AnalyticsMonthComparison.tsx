...
export function AnalyticsMonthComparison({
  monthlyData,
}: AnalyticsMonthComparisonProps) {
  ...
  const diff = dataA && dataB ? dataA.appointments - dataB.appointments : 0;
  const pctChange =
    dataA && dataB && dataB.appointments !== 0
      ? ((dataA.appointments - dataB.appointments) / Math.abs(dataB.appointments)) * 100
      : 0;
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="rounded-lg border p-4">
                ...
                <p className="text-lg font-bold">{formatNumber(dataA.appointments)}</p>
                <div className="flex gap-4 mt-2 text-xs text-muted-foreground">
                  <span>مكتمل: {formatNumber(dataA.completed)}</span>
                  <span>مرضى جدد: {formatNumber(dataA.newPatients)}</span>
                </div>
              </div>
              <div className="rounded-lg border p-4">
                <p className="text-xs text-muted-foreground mb-1">
                ...
                <p className="text-lg font-bold">{formatNumber(dataB.appointments)}</p>
                <div className="flex gap-4 mt-2 text-xs text-muted-foreground">
                  <span>مكتمل: {formatNumber(dataB.completed)}</span>
                  <span>مرضى جدد: {formatNumber(dataB.newPatients)}</span>
                </div>
              </div>
            </div>
