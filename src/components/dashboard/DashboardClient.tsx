...
export default function DashboardClient({
  ...
  const apptOpts = doctorId ? { doctorId } : undefined;
  const { data: todayApps, isLoading: appsLoading } = useAppointments(startOfDay(today), endOfDay(today), apptOpts);
    ...
    <div className="min-h-full pb-8">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-4 md:gap-8">
        ...
        <div className="xl:col-span-3 flex flex-col gap-4 md:gap-8">
          ...
          {/* ── Today's Timeline + Upcoming / Receptionist Links ── */}
          {isReceptionist ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
              <button
                onClick={() => router.push("/dashboard/calendar")}
                className="flex flex-col items-center justify-center gap-3 p-6 md:p-8 bg-white rounded-xl md:rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:border-emerald-200 transition-all group"
              >
                <div className="w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-500 flex items-center justify-center shadow-md group-hover:scale-105 transition-transform">
                  <CalendarDays className="w-7 h-7 md:w-8 md:h-8 text-white" />
                </div>
                <span className="text-sm md:text-base font-bold text-slate-800">تقويم المواعيد</span>
                <span className="text-[10px] md:text-xs text-slate-400">عرض وإدارة جميع المواعيد</span>
              </button>
              <button
                onClick={() => router.push("/dashboard/waitlist")}
                className="flex flex-col items-center justify-center gap-3 p-6 md:p-8 bg-white rounded-xl md:rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:border-amber-200 transition-all group"
              >
                <div className="w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-gradient-to-br from-amber-500 to-orange-500 flex items-center justify-center shadow-md group-hover:scale-105 transition-transform">
                  <Clock className="w-7 h-7 md:w-8 md:h-8 text-white" />
                </div>
                <span className="text-sm md:text-base font-bold text-slate-800">قائمة الانتظار</span>
                <span className="text-[10px] md:text-xs text-slate-400">متابعة المرضى في الانتظار</span>
              </button>
            </div>
          ) : (
          ...
          <AppointmentsChart monthlyAppointments={monthlyAppointments} />
        </div>
...
function AppointmentsChart({ monthlyAppointments }: { monthlyAppointments: MonthlyCount[] }) {
  const [anim, setAnim] = useState(false);
  useEffect(() => { setTimeout(() => setAnim(true), 300); }, []);

  const max = Math.max(...monthlyAppointments.map((m) => m.count), 1);
    ...
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-800">نظرة على المواعيد</h3>
            <p className="text-[10px] text-slate-400">آخر ٦ أشهر</p>
          </div>
        </div>
      </div>
      <div className="flex items-end gap-2.5 h-44">
        {monthlyAppointments.map((m, i) => {
          const pct = anim ? (m.count / max) * 100 : 0;
          const barColor = i % 2 === 0 ? "bg-gradient-to-t from-blue-500 to-blue-400" : "bg-gradient-to-t from-indigo-500 to-indigo-400";
          return (
            <div key={m.label} className="flex-1 flex flex-col items-center gap-2 h-full justify-end">
              <span className={`text-[9px] font-bold ${m.count >= max * 0.7 ? "text-slate-700" : "text-transparent"} transition-all`}>
                {m.count}
              </span>
              ...
              <span className="text-[10px] font-semibold text-slate-400 mt-1">{m.label}</span>
            </div>
          );
        })}
      </div>
